...
**Communication:** SDK uses **Eclipse Zenoh** pub/sub middleware for real-time client-daemon communication.
...
**Two motion control modes:**
- `goto_target()` — smooth interpolated motion (default, for gestures ≥0.5s). Interpolation methods: `linear`, `minjerk` (default), `ease`, `cartoon`.
...
- For headless MuJoCo in CI: `MUJOCO_GL=disable`.
